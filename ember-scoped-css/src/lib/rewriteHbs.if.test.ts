...
describe('if in a class attribute', () => {
  it('renames both branches, unquoted', () => {
    expect(rewrite('<div class={{if x "a" "b"}}></div>')).to.equal(
      '<div class={{if x "a_pfx" "b_pfx"}}></div>',
    );
  });

  it('renames both branches, quoted', () => {
    expect(rewrite('<div class="{{if x "a" "b"}}"></div>')).to.equal(
      '<div class="{{if x "a_pfx" "b_pfx"}}"></div>',
    );
  });
  ...
  it('still leaves the condition alone when the element is itself scoped', () => {
    ...
    expect(
      rewriteWithScopedDiv('<div class={{if (eq this.m "a") "a" "b"}}></div>'),
    ).to.equal(
...
describe('deep if', () => {
  ...
  it('leaves a param not in the CSS alone inside a concat branch', () => {
    expect(
      rewrite('<div class={{if x (concat "a" " " "global-thing") "b"}}></div>'),
    ).to.equal(
