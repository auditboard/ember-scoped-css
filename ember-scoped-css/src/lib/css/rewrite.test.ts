...
function scopeSelector(css) {
  return rewriteCss(css, 'postfix', 'foo.css')
    .replace('/* foo.css */\n', '')
    .replace(/\s*{[^}]*}\s*$/, '')
    .trim();
}
...
describe('attribute selectors', () => {
  it('scopes a bare attribute selector with a marker class', () => {
  ...
  describe('class-targeting operators', () => {
    it('rewrites the value for ~= (Bucket A, no marker)', () => {
      expect(scopeSelector('[class~="foo"] { color: red; }')).to.equal(
        '[class~="foo_postfix"]',
      );
    });

    it('rewrites every token for = (Bucket A, no marker)', () => {
      expect(scopeSelector('[class="foo bar"] { color: red; }')).to.equal(
        '[class="foo_postfix bar_postfix"]',
      );
    });

    it('keeps the value and adds a marker for ^= (Bucket B)', () => {
      expect(scopeSelector('[class^="foo"] { color: red; }')).to.equal(
        '[class^="foo"].postfix',
      );
    });
